...
In this step, the [data source](/content/sql/ddl/create-data-source#sql-create-data-source) used to
...
* **Create New Data Source** - click to create a new
  ...
  * **Auth Type** - select an authorization scheme; this will
    ...
    schemes, create a [Kafka Credential](/content/sql/ddl/create-credential#sql-create-credential)
    via SQL (in a [Workbook](/content/admin/workbench/workbooks), for
    example) and then create a
    [Kafka Data Source](/content/sql/ddl/create-data-source#sql-create-data-source) that uses the
    credential
    </Info>
...
* **Properties** - check any properties that should apply to this
  ...
  * **Text Search** - make this column
    ...
    [FILTER_BY_STRING](/content/sql/query/string-functions#sql-string-functions-fts) in *search* mode
