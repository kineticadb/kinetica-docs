...
| `proc_data.request_info` | Map of strings to strings | Returns a map of basic information about the [execute_proc()](/content/api/python/source/gpudb/execute-proc#gpudb.GPUdb.execute_proc) request, map values being accessed using: <br/> <br/> `proc_data.request_info[<map_key>]` <br/> |
...
<a id="status-reporting-1"></a>
## Status Reporting


The `proc_data.status` property can be set to a string value to help
convey status information during UDF execution, e.g.,
`proc_data.status="25% complete"`. The [show_proc_status()](/content/api/python/source/gpudb/show-proc-status#gpudb.GPUdb.show_proc_status) function will
...
* If the user-provided UDF code is executing and has set a status message,
  [show_proc_status()](/content/api/python/source/gpudb/show-proc-status#gpudb.GPUdb.show_proc_status) will return the last message that was set.
...
* If the UDF is killed while executing user-provided UDF code,
  [show_proc_status()](/content/api/python/source/gpudb/show-proc-status#gpudb.GPUdb.show_proc_status) will return the last message that was set.
* If the user-provided UDF code errors out, [show_proc_status()](/content/api/python/source/gpudb/show-proc-status#gpudb.GPUdb.show_proc_status) will return
