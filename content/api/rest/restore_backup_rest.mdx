...
<ParamField body="directory">

KiFS [File directory(ies)](../../tools/kifs/).
...
[Graph](../../graph_solver/network_graph_solver/) definition(s). Source table(s), if applicable, are required in order to restore graph objects.
...
<ParamField body="restore_all_permissions">

Whether or not all permissions of restored principals should be restored or scoped to the restored objects.
...
<ParamField body="target_schema_map">

Restore schema-based objects to alternate schema. Value is a comma delimitted list of key:value pairs mapping the original (source) schema name as it exists in the backup to a target (destination) schema namespace: "src: dst [, src: dst [, ...]]". Note that schema names are case sensitive and must adhere to the database schema [naming criteria](../../concepts/schemas/).
