...
File data for the files being uploaded, for the respective files in input parameter *file_names*.
...
If *true*, any existing files specified in input parameter *file_names* will be deleted prior to start of upload. Otherwise the file is replaced once the upload completes.  Rollback of the original file is no longer possible if the upload is cancelled, aborted or fails if the file was deleted beforehand.
...
'upload_files_response' or 'none' in case of an error
