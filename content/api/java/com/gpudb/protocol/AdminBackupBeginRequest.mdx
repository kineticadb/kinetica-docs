...
<div className="block">
A set of parameters for <a href="/content/api/java/com/gpudb/GPUdb#adminBackupBegin(com.gpudb.protocol.AdminBackupBeginRequest)"><code>GPUdb.adminBackupBegin</code></a>. 
<p> Prepares the system for a backup by closing all open file handles after allowing current active jobs to complete. When the database is in backup mode, queries that result in a disk write operation will be blocked until backup mode has been completed by using <a href="/content/api/java/com/gpudb/GPUdb#adminBackupEnd(com.gpudb.protocol.AdminBackupEndRequest)"><code>GPUdb.adminBackupEnd</code></a>.</p>
</div>
</div>
