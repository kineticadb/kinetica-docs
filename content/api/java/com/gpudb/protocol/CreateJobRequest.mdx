...
<div className="inheritance">com.gpudb.protocol.CreateJobRequest</div>
</div>
...
<div className="block">
A set of parameters for <a href="/content/api/java/com/gpudb/GPUdb/createjob#createJob(com.gpudb.protocol.CreateJobRequest)"><code>GPUdb.createJob</code></a>. 
<p> Create a job which will run asynchronously. The response returns a job ID, which can be used to query the status and result of the job. The status and the result of the job upon completion can be requested by <a href="/content/api/java/com/gpudb/GPUdb/getjob#getJob(com.gpudb.protocol.GetJobRequest)"><code>GPUdb.getJob</code></a>.</p>
</div>
</div>
