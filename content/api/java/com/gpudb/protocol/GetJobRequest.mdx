...
<div className="block">
A set of parameters for <a href="/content/api/java/com/gpudb/GPUdb#getJob(com.gpudb.protocol.GetJobRequest)"><code>GPUdb.getJob</code></a>. 
<p> Get the status and result of asynchronously running job. See the <a href="/content/api/java/com/gpudb/GPUdb#createJob(com.gpudb.protocol.CreateJobRequest)"><code>GPUdb.createJob</code></a> for starting an asynchronous job. Some fields of the response are filled only after the submitted job has finished execution.</p>
</div>
</div>
