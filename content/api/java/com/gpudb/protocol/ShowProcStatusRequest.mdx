...
<div className="block">
A set of parameters for <a href="/content/api/java/com/gpudb/GPUdb#showProcStatus(com.gpudb.protocol.ShowProcStatusRequest)"><code>GPUdb.showProcStatus</code></a>. 
<p> Shows the statuses of running or completed proc instances. Results are grouped by run ID (as returned from <a href="/content/api/java/com/gpudb/GPUdb#executeProc(com.gpudb.protocol.ExecuteProcRequest)"><code>GPUdb.executeProc</code></a>) and data segment ID (each invocation of the proc command on a data segment is assigned a data segment ID).</p>
</div>
</div>
