...
<div className="block">
A set of parameters for <a href="/content/api/java/com/gpudb/GPUdb#adminAddRanks(com.gpudb.protocol.AdminAddRanksRequest)"><code>GPUdb.adminAddRanks</code></a>. 
<p> Add one or more ranks to an existing Kinetica cluster. The new ranks will not contain any data initially (other than replicated tables) and will not be assigned any shards. To rebalance data and shards across the cluster, use <a href="/content/api/java/com/gpudb/GPUdb#adminRebalance(com.gpudb.protocol.AdminRebalanceRequest)"><code>GPUdb.adminRebalance</code></a>. </p>
...
<p> This endpoint's processing includes copying all replicated table data to the new rank(s) and therefore could take a long time. The API call may time out if run directly. It is recommended to run this endpoint asynchronously via <a href="/content/api/java/com/gpudb/GPUdb#createJob(com.gpudb.protocol.CreateJobRequest)"><code>GPUdb.createJob</code></a>.</p>
</div>
</div>
