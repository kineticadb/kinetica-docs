...
<div className="inheritance">com.gpudb.filesystem.MultiPartUploadInfo</div>
</div>
...
<div className="block">
This class packages and mimics certain important attributes of a multi-part upload operation. The attributes of this class are in one-one correspondence to those specified in the <a href="/content/api/java/com/gpudb/GPUdb/uploadfiles#uploadFiles(java.util.List,java.util.List,java.util.Map)"><code>GPUdb.uploadFiles(List, List, Map)</code></a> endpoint. 
<p>This class is read-only for API consumers - instances are created internally and provided via Result callbacks. Users should not attempt to instantiate this class directly.</p>
</div>
</div>
