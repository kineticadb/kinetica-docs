...
with the [CREATE CREDENTIAL](/content/sql/ddl/create-credential#sql-create-credential) command, and
...
[CREATE DATA SOURCE](/content/sql/ddl/create-data-source#sql-create-data-source) command in Kinetica.
...
by using the [ALTER TABLE](/content/sql/ddl/alter-table#sql-alter-table-manage-sub) command.
...
[date and time format codes](/content/sql/query/conversion-functions#sql-datetime-conversion-codes).
...
1. Create [credentials](/content/sql/ddl/create-credential#sql-create-credential) for both the Kafka
   source and the Schema Registry service.
2. Create a [data source](/content/sql/ddl/create-data-source#sql-create-data-source) that references
