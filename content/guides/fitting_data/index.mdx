...
2. Expansion -- using [iteration](/content/sql/query/iteration#sql-iteration) to divide
...
<VideoEmbed id="oLYIPBRteEM" thumb="/content/guides/fitting_data/video-graph-modeling.jpg" title="Graph Modeling Concepts" caption="Model a graph with four components" />
...
2. Copy the following [CREATE SCHEMA](/content/sql/ddl/create-schema#sql-create-schema) statement into the
...
* The [EDGES](/content/graph_solver/network_graph_solver#graph-edges) are derived from WKT LINESTRINGs in the
  ...
  (`AS ID`).
* <VideoEmbed id="ouZb00xEzh8" thumb="/content/guides/fitting_data/video-graph-weights.jpg" title="Weighted Graphs" caption="Modeling data for a weighted graph in Kinetica" />
...
through [iteration](/content/sql/query/iteration#sql-iteration), wherein each source record
...
1. On the top menu, click **Query --&gt; SQL** to go to the
   *SQL Tool* page.
2. Copy the following [CREATE TABLE](/content/sql/ddl/create-table#sql-create-table) statement into the
   **SQL Statements** text area on the top section of the page:
   ```sql
...
This [CREATE TABLE ... AS](/content/sql/ddl/create-table-as#sql-create-table-as) statement performs several actions
