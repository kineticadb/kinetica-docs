...
The [tutorial script](https://raw.githubusercontent.com/kineticadb/kinetica-docs/master/content/examples/sql/sql_tutorial.kisql) makes
reference to a [data file](https://raw.githubusercontent.com/kineticadb/kinetica-docs/master/content/examples/data/taxi_trip_data.csv)
...
[schema](/content/sql/ddl/create-schema#sql-create-schema) needs to be created to contain them:
...
[Tables](/content/sql/ddl/create-table#sql-create-table) can be created as
...
The [INSERT INTO ... VALUES](/content/sql/dml/insert#sql-dml-insert-values) statement is used to insert records
into a [table](/content/sql/ddl/create-table#sql-create-table). At a minimum, each record must
...
[KiFS](/content/tools/kifs) and then inserted into the target table from
there:
...
The [UPDATE](/content/sql/dml/update#sql-update) statement is used to update matching key values
...
The [DELETE](/content/sql/dml/delete#sql-delete) statement is used to delete records from a
...
[table](/content/sql/ddl/create-table#sql-create-table) creation, including *indexes* and
*dictionary encoding*. Use the [ALTER TABLE](/content/sql/ddl/alter-table#sql-alter-table) command to specify
...
[ADD INDEX](/content/sql/ddl/alter-table#sql-alter-table-column-index-add) will add a column
...
Use [ALTER COLUMN](/content/sql/ddl/alter-table#sql-alter-table-column-modify) to add column
properties like
...
[SQL functions](/content/sql/query/array-functions#sql-functions). Aggregate functions are paired
...
<a id="common-table-expression-with"></a>
## Common Table Expression / With


The [WITH](/content/sql/query/with-common-table-expressions#sql-with) statement can be used to give a subquery an
...
[Joins](/content/sql/query/join#sql-join) allow you to link multiple *tables* together,
...
[CREATE TABLE ... AS](/content/sql/ddl/create-table-as#sql-create-table-as). These can also be created as temporary
...
<a id="union-intersect-and-except"></a>
## Union, Intersect, and Except


A [UNION](/content/sql/query/set-operations#sql-union) can be used to combine homogeneous data sets into one
...
A [INTERSECT](/content/sql/query/set-operations#sql-intersect) will combine data sets but only include the
...
A [EXCEPT](/content/sql/query/set-operations#sql-except) will return records that appear in the first data
...
Use [TRUNCATE TABLE](/content/sql/ddl/truncate-table#sql-truncate-table) to remove all records from a *table*
...
* [SQL Script](https://raw.githubusercontent.com/kineticadb/kinetica-docs/master/content/examples/sql/sql_tutorial.kisql)
* [SQL Data File (taxi trip data)](https://raw.githubusercontent.com/kineticadb/kinetica-docs/master/content/examples/data/taxi_trip_data.csv)
* [SQL Output](https://raw.githubusercontent.com/kineticadb/kinetica-docs/master/content/examples/sql/sql_tutorial.out)
