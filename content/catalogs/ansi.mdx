...
<a id="constraint-column-usage"></a>
...
| `TABLE_NAME` | `VARCHAR(256)` | Name of this *SQL-GPT context rule* object, denoting the type of *context rule* it is. <br/> <br/> `rules`: Globally-applicable rule definition <br/> <br/> *&lt;schema_name.table_name&gt;*: Table-specific rule definition |
...
*table definition* and both the [column comment](/content/sql/ddl/comments#sql-comment) from
...
<a id="information-schema-catalog-name"></a>
...
[data sink](/content/sql/ddl/create-data-sink#sql-create-data-sink) through which the backup was
created.
...
| `STATUS` | `VARCHAR(32)` | Current status of the back up processing. <br/> <br/> `backup_complete`: Back up complete. <br/> <br/> `backup_in_progress`: Back up in progress. <br/> <br/> `backup_failed`: Back up failed. <br/> <br/> `unknown`: Back up in unknown state. <br/> <br/> `validated`: Back up request validated, but not started yet. |
...
| `DATASOURCE_KIND` | `VARCHAR(16)` | Type of the *data source*; see [CREATE DATA SOURCE](/content/sql/ddl/create-data-source#sql-create-data-source) for supported providers. |
...
[data source](/content/sql/ddl/create-data-source#sql-create-data-source) and the indexes of the most
...
| `DATASOURCE_NAME` | `VARCHAR(256)` | Name of the [data source](/content/sql/ddl/create-data-source#sql-create-data-source) or [data sink](/content/sql/ddl/create-data-sink#sql-create-data-sink) used for this data transfer (for those that make use of either). |
...
<a id="ki-query-span-metrics-by-sql-step"></a>
...
[data source](/content/sql/ddl/create-data-source#sql-create-data-source) through which the backup was
restored.
...
| `BACKUP_KIND` | `VARCHAR(64)` | Type of backup this is. <br/> <br/> `full`: Backup of all data, system-wide. <br/> <br/> `differential`: Backup of all new or modified data since the last full back up. <br/> <br/> `incremental`: Backup of all new or modified data since the last back up of any type. |
| `STATUS` | `VARCHAR(32)` | Current status of the restoration processing. <br/> <br/> `restore_complete`: Restoration complete. <br/> <br/> `restore_in_progress`: Restoration in progress. <br/> <br/> `restore_failed`: Restoration failed. <br/> <br/> `unknown`: Restoration in unknown state. <br/> <br/> `validated`: Restoration request validated, but not started yet. |
...
| `DATASINK_NAME` | `VARCHAR(256)` | Name of the [table](/content/sql/ddl/create-table#sql-create-table)/[data sink](/content/sql/ddl/create-data-sink#sql-create-data-sink) targeted by this stream.  A stream targeting a table directly will have an auto-generated data sink name. |
...
[materialized view](/content/sql/ddl/create-materialized-view#sql-create-materialized-view); for example, a
...
| `SOURCE_TABLE_KIND` | `VARCHAR(1)` | Type of the source table depended on. <br/> <br/> E: External table <br/> <br/> H: Partitioned table <br/> <br/> I: Materialized view intermediate table <br/> <br/> M: Materialized view <br/> <br/> R: Regular (local) table |
...
| `OBJECT_TYPE` | `VARCHAR(256)` | Type of object grant this mapping represents (for object-level permissions). <br/> <br/> `context`: [SQL-GPT context](/content/sql/sqlgpt) <br/> <br/> `credential`: [Credential](/content/sql/ddl/create-credential#sql-create-credential) <br/> <br/> `datasink`: [Data sink](/content/sql/ddl/create-data-sink#sql-create-data-sink) <br/> <br/> `datasource`: [Data source](/content/sql/ddl/create-data-source#sql-create-data-source) <br/> <br/> `directory`: [KiFS directory](/content/sql/kifs#sql-kifs-create-dir) <br/> <br/> `graph`: [Graph](/content/sql/graph) <br/> <br/> `proc`: [SQL procedure](/content/sql/procedure) or [UDF/UDTF](/content/sql/udf) <br/> <br/> `schema`: [Schema](/content/sql/ddl/create-schema#sql-create-schema) <br/> <br/> `table`: [Table](/content/sql/ddl/create-table#sql-create-table) or [view](/content/sql/ddl/create-view#sql-create-view) <br/> <br/> `table_monitor`: [Stream](/content/sql/ddl/create-stream#sql-create-stream) |
...
| `STORAGE_TIER` | `VARCHAR(32)` | Tier containing this partition. <br/> <br/> `PERSIST`: [Persist tier](/content/rm/concepts#rm-concepts-tiers-persist) <br/> <br/> `RAM`: [RAM tier](/content/rm/concepts#rm-concepts-tiers-ram) |


<a id="referential-constraints"></a>
...
[schema](/content/sql/ddl/create-schema#sql-create-schema) in the database.
...
| `CONSTRAINT_TYPE` | `VARCHAR(16)` | Type of constraint; one of: <br/> <br/> * `PRIMARY KEY` <br/> * `FOREIGN KEY` |
| `IS_DEFERRABLE` | `BOOLEAN` | Whether the constraint is deferrable. <br/> <br/> false: Constraint is a *primary key*, non-deferrable in *Kinetica*. <br/> <br/> true: Constraint is a *foreign key*, deferrable in *Kinetica*. |
| `INITIALLY_DEFERRED` | `BOOLEAN` | Whether the constraint is initially deferred. <br/> <br/> false: Constraint is a *primary key*, not deferred in *Kinetica*. <br/> <br/> true: Constraint is a *foreign key*, deferred in *Kinetica*. |
| `ENFORCED` | `BOOLEAN` | Whether the constraint is enforced. <br/> <br/> false: Constraint is a *foreign key*, unenforced in *Kinetica*. <br/> <br/> true: Constraint is a *primary key*, enforced in *Kinetica*. |
| `COMMENT` | `VARCHAR` | Not used. |
| `CREATED` | `TIMESTAMP` | Timestamp of constraint creation. |
| `LAST_ALTERED` | `TIMESTAMP` | Timestamp of the most recent constraint update (initially, time of creation). |
...
| `TABLE_TYPE` | `VARCHAR(16)` | Table/view type. <br/> <br/> `BASE TABLE`: Regular (local) table or external table <br/> <br/> `VIEW`: Logical or materialized view |
...
[view](/content/sql/ddl/create-view#sql-create-view) within the database, including those in
