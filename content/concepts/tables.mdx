...
of *partitioning* scheme.  The lone exception is
[manual list partitioning](/content/concepts/tables#partitioning-by-list-manual), in which
a *partition* can explicitly claim *null* as one of its values; records with a
*null* *partition key* value are then placed in that *partition* instead of the
*default partition*.
</Note>
...
* [Creating a partitioned table](/content/sql/ddl/create-table#sql-create-table-partition)
* [Altering a table's partitions](/content/sql/ddl/alter-table#sql-alter-table-partition-add)
...
A *partition* can include *null* among its list of values, in which case all
records with a *null* *partition key* value will be assigned to that
*partition*, rather than to the *default partition*.  At most one *partition*
may claim *null*.
...
<Note>
Unlike the other *partitioning* schemes, a
*series partition key* must be a base column--column expressions cannot be
used.
</Note>
...
[CREATE TABLE](/content/sql/ddl/create-table#sql-create-table).
...
<a id="relation-to-shard-key-1"></a>
### Relation to Shard Key
...
<a id="relation-to-foreign-key-1"></a>
### Relation to Foreign Key
...
<a id="relation-to-primary-key-1"></a>
### Relation to Primary Key
...
<a id="relation-to-shard-key-2"></a>
### Relation to Shard Key
