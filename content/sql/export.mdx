...
* a [data sink](/content/sql/ddl/create-data-sink#sql-create-data-sink) configured to allow write
  access to:
...
| `DATA SINK` | Table | Export data through the given [data sink](/content/sql/ddl/create-data-sink#sql-create-data-sink).  [Data sink connect privilege](/content/sql/security#sql-security-priv-mgmt-dsink-grant) is required when exporting through a *data sink*. <br/> <Info> Local table *data sinks* are not supported. </Info> |
...
<a id="export-table-to-file-kifs"></a>
...
<a id="export-query-to-file-kifs"></a>
