...
<Accordion title="AVRO_SCHEMA" id="avro_schema" defaultOpen>

String containing the *Avro* schema to use when reading *Avro* data that includes only
records--i.e. data that does not embed its own schema.


<Info>
Supplying this option implies the source data is schemaless.
</Info>

</Accordion>

<Accordion title="AVRO_SCHEMA_NO_INFERENCE" id="avro_schema_no_inference" defaultOpen>

Whether to derive the target table's column types solely from the *Avro* schema, when
one is available, instead of inferring them from the data.
...
Choose a comma-separated list of fields from the source file(s) to exclude from the
load.  Fields can be identified by either `NAME` or `POSITION`.  If ignoring by
`NAME`, the specified names must match the source file field names exactly; source
fields can be named with `FIELDS MAPPED AS COLUMN`.
...
<Info>
* If the *external table* has no column definition, the non-ignored source fields
  define its columns, in source file order.
* If the *external table* has a column definition, the non-ignored source fields are
  matched to its columns by name, case-insensitively; their order & number need
  not correspond to those columns, and any source field matching none of them is
  ignored as well.
* If the *external table* has a column definition and the source data has no field
  names (no header row and no `FIELDS MAPPED AS COLUMN`), the non-ignored source
  fields are matched to its columns by position instead, and the source field
  count must equal the column count plus the number of fields ignored.
* Ignoring a source field that corresponds to a *external table* column causes that
  column to be populated with its default value, `init_with_*` value, or null.
  This makes `FIELDS IGNORED BY` the means of preferring a column default over a
  value present in the source data.
* Ignoring fields by `POSITION` is only supported for delimited text files.
</Info>

</Accordion>

<Accordion title="FIELDS MAPPED AS COLUMN" id="fields-mapped-as-column" defaultOpen>

Choose a comma-separated list of names to assign to the source file fields, in order;
the i-th name is assigned to the i-th field of the file.  Only supported for delimited
text files.


```
FIELDS MAPPED AS COLUMN(ID, Name, Category, Stock)
```
Naming the source fields allows them to be matched to the *external table* columns by
name, and allows `FIELDS MAPPED BY NAME` & `FIELDS IGNORED BY NAME`, which
otherwise require the source file to have a header row.


<Info>
* If the source file has a header row, these names override the header names; if it
  has no header row, these names become the source field names.
* For a source file with no header row, applying this option switches the matching
  of source fields to *external table* columns from positional to by-name.  Any
  column with no matching source field is then populated with its default value or
  null, rather than being filled positionally.
* The list is not validated against the number of fields in the file; if it is
  shorter, the trailing source fields are left unnamed.
</Info>
...
Choose a comma-separated list of source file fields to load, in the order of the
*external table* columns into which they will be loaded; the i-th entry identifies the
source field that feeds the i-th column.  This is a positional mapping onto the
*external table* rather than a filter, so it can also be used to reorder source fields.
Fields can be identified by either `NAME` or `POSITION`, but the two cannot be
mixed--a single non-numeric entry causes the entire list to be read as names.

* Identifying by Name - the source file fields `ID`, `Name`, & `Stock` are
  loaded into the first three columns, respectively:

  ```
  FIELDS MAPPED BY NAME(ID, Name, Stock)
  ```
* Identifying by Position - the 1st, 2nd, & 5th source file fields are loaded into
  the first three columns, respectively:

  ```
  FIELDS MAPPED BY POSITION(1, 2, 5)
  ```
* Positions are 1-based & may be given as ranges, which may descend to reverse the
  order of that group, while an empty entry is a placeholder that feeds no source
  field to the corresponding column.  Here, the 5th source field is loaded into the
  first column, the 7th into the second, nothing into the third, and the 1st through
  3rd into the fourth through sixth:

  ```
  FIELDS MAPPED BY POSITION(5, 7, , 1..3)
  ```

<Info>
* If the *external table* has no column definition, it is created with the identified
  fields as its columns, in the order given; the list may identify any subset of
  the source fields.
* If the *external table* has a column definition, the number of entries must equal its
  column count; empty entries can be used to pad the list to that width.
* Applying this option disables the population of *external table* columns that no
  source field feeds; such columns are assigned null instead of their default
  value, `init_with_now`, or `init_with_uuid` value, and the record is rejected
  if the column is not nullable.  To have unfed columns take their defaults, use
  `FIELDS IGNORED BY` instead.
* If mapping by `NAME`, the specified names must match the source file field
  names exactly; source fields can be named with `FIELDS MAPPED AS COLUMN`.
* Mapping fields by `POSITION` is only supported for delimited text & Avro files.
</Info>
...
<Info>
This option is only available for file-based *data sources*.
</Info>
