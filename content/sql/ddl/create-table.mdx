...
<Note>
An *interval-partitioned* *table* can have only one
*partition key*.
</Note>
...
<Note>
Each *series partition key* must be a base column; column
expressions cannot be used.
</Note>


<Note>
A *series-partitioned* *table* must either be
[replicated](/content/concepts/tables#replication) or have its *shard key* columns be a
subset of its *partition key* columns.
</Note>
