...
* [Loading server-accessible data sets](/content/sql/load#sql-load-file-server) via
  ...
  * can be reached by *Kinetica* via an
    [external data source](/content/sql/ddl/create-data-source#sql-create-data-source) object
...
* a [data source](/content/sql/ddl/create-data-source#sql-create-data-source) configured to allow
  access to:
...
See [Manage Subscription](/content/sql/ddl/alter-table#sql-alter-table-manage-sub) for pausing, resuming, &
...
Name of the [schema](/content/sql/ddl/create-schema#sql-create-schema) containing the table into which data will be
loaded
...
Name of the [table](/content/sql/ddl/create-table#sql-create-table) into which data will be loaded
...
<Info>
* If the target table has no column definition, the non-ignored source fields
  define its columns, in source file order.
* If the target table has a column definition, the non-ignored source fields are
  ...
  ignored as well.
* If the target table has a column definition and the source data has no field
...
* Ignoring a source field that corresponds to a target table column causes that
...
```
Naming the source fields allows them to be matched to the target table columns by
...
* For a source file with no header row, applying this option switches the matching
  of source fields to target table columns from positional to by-name.  Any
...
target table columns into which they will be loaded; the i-th entry identifies the
source field that feeds the i-th column.  This is a positional mapping onto the
target table rather than a filter, so it can also be used to reorder source fields.
...
<Info>
* If the target table has no column definition, it is created with the identified
  fields as its columns, in the order given; the list may identify any subset of
  the source fields.
* If the target table has a column definition, the number of entries must equal its
  column count; empty entries can be used to pad the list to that width.
* Applying this option disables the population of target table columns that no
...
<a id="delimited-text-options-1"></a>
...
<a id="insert-into-select-from-file"></a>
...
Name of the [schema](/content/sql/ddl/create-schema#sql-create-schema) containing the *table* into which data will
be inserted
...
Name of the [table](/content/sql/ddl/create-table#sql-create-table) into which data will be inserted
