...
<a id="odbc-jdbc-configuration"></a>
...
| `InitialConnectionTimeoutSeconds` | `-1` | Seconds the driver will spend retrying the initial connection to the database before giving up.  `0` disables retrying, in which case the URLs supplied are used as given, without auto-discovery.  A non-zero value retries with a backoff that starts at one minute and doubles with each attempt.  The default of `-1` defers to the API default of `0`. *(JDBC only)* |
